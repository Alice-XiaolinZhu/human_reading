...
def forward(batch, calculateAccuracy=False):
    ...
    if WITH_LM:
        ...
        targets = torch.where(attentionDecisions == 1.0, torch.LongTensor(targets.detach().numpy()).cuda(), torch.zeros(attentionDecisions.size()).cuda()) # 0: mask
        ...
        print("??????", torch.LongTensor(targets.detach().numpy()).cuda())
