...
def forward(batch, calculateAccuracy=False):
    ...
    if WITH_LM:
        ...
        targets_mask = torch.where(attentionDecisions == 1.0, targets, torch.zeros(attentionDecisions.size()).cuda()) # 0: mask
        targets = torch.cat([targets_mask, targets_mask], dim=0)
